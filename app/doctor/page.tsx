...
// Define the modules for the doctor path
const doctorModules = [
  {
    id: 1,
    title: "Biomechanics and Body Movement",
    description: "Study of mechanical principles governing human movement and body mechanics",
    topics: [
      "Kinematics and Kinetics",
      "Body Levers and Fulcrums",
      "Center of Gravity",
      "Movement Analysis",
      "Injury Prevention",
    ],
  ...
  },
  {
    ...
    description: "Physics principles behind medical imaging technologies and radiation safety",
    topics: ["X-ray Physics", "CT and MRI Principles", "Ultrasound Physics", "Nuclear Medicine", "Radiation Safety"],
    duration: "60 min",
  },
  {
    id: 3,
    title: "Fluid Dynamics in Circulatory Systems",
    description: "Understanding blood flow and cardiovascular physics principles",
    topics: [
      "Poiseuille's Law",
  ...
  },
  {
    ...
    description: "Physics of light and the biological mechanisms of human vision",
    topics: ["Light Refraction", "Eye Optics", "Photoreceptors", "Visual Processing", "Vision Correction"],
    duration: "55 min",
  },
  {
    id: 5,
    title: "Bioelectricity and Neural Signals",
    description: "Electrical processes in biological systems and neural communication",
    topics: [
      "Action Potentials",
      "Ion Channels",
...
export default function DoctorPathPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        for (const module of doctorModules) {
          const isCompleted = userProgress?.completed_modules?.includes(module.id) || false
          const isUnlocked = await isModuleUnlocked(currentUser.id, "doctor", module.id)
  ...
  return (
    <div className="container py-8 px-4 mx-auto max-w-6xl">
      <MotionDiv initial="hidden" animate="visible" variants={fadeIn} className="mb-8">
        <h1 className="text-4xl font-bold mb-4 gradient-heading">Physics for Doctors</h1>
        <p className="text-lg text-muted-foreground max-w-3xl">
          Explore the physics principles essential for medical practice through our specialized modules designed for
              ...
              return (
                ...
                >
                  <Card className={`module-card ${!status.isUnlocked ? "locked" : ""}`}>
                    ...
                    <CardFooter>
                      {status.isUnlocked ? (
                        <Link href={`/doctor/module/${module.id}`} className="w-full">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm text-muted-foreground">
                        {progress?.completed_modules?.length || 0}/{doctorModules.length} modules
                      </span>
                    </div>
                    <div className="h-2 bg-secondary rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${((progress?.completed_modules?.length || 0) / doctorModules.length) * 100}%`,
                        }}
                      ></div>
                    </div>
                  </div>
